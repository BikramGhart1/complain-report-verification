...
export const getUserComplaints = async ({ page = 1, limit = 10, status = '', search = '' } = {}) => {
...
export const submitComplaint = async (form, evidenceFile) => {
  ...
  fd.append('victim_name',          `${form.victimFirstName} ${form.victimLastName}`.trim());
  fd.append('victim_phone_number',  form.victimPhone);
  fd.append('perpetrator_name',     form.suspectName);
  fd.append('crime_location',       `${form.suspectPlatform} — ${form.suspectProfileUrl}`.trim());
